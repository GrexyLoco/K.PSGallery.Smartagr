...
function New-SmartRelease {
    ...
    begin {
        ...
        Write-SafeInfoLog -Message "Starting smart release creation" -Additional @{ "Details" = "TargetVersion: $TargetVersion" }
    ...
    process {
        try {
            ...
            if (-not $SkipGitHubRelease) {
                Write-SafeInfoLog -Message "Step 1: Creating GitHub draft release" -Additional @{ "Details" = "Version: $normalizedVersion" }
                ...
                if ($PSCmdlet.ShouldProcess($normalizedVersion, "Create GitHub draft release")) {
                    ...
                    if ($draftResult.Success) {
                        ...
                        Write-SafeInfoLog -Message "Draft release created successfully" -Additional @{ "Details" = "ReleaseId: $($draftResult.ReleaseId)" }
                    } else {
                    ...
                    Write-SafeInfoLog -Message "Draft release simulation successful" -Additional @{ "Details" = "ReleaseId: $($mockDraftResult.ReleaseId) (WhatIf)" }
                }
            } else {
                $result.StepResults.DraftCreation.Success = $true
            ...
            if ($result.StepResults.DraftCreation.Success) {
                Write-SafeInfoLog -Message "Step 2: Creating smart tags" -Additional @{ "Details" = "Version: $normalizedVersion" }
                ...
                if ($WhatIfPreference) {
                    ...
                    Write-SafeInfoLog -Message "Smart tags simulation successful" -Additional @{ "Details" = "Tags: $($mockTagResult.TagsCreated -join ', ') (WhatIf)" }
                } else {
                    # Real execution
                    ...
                    if ($tagResult.Success) {
                        ...
                        Write-SafeInfoLog -Message "Smart tags created successfully" -Additional @{ "Details" = "Tags: $($tagResult.TagsCreated -join ', ')" }
                    } else {
            ...
            if ($result.StepResults.TagCreation.Success -and $result.ReleaseDraftCreated) {
                Write-SafeInfoLog -Message "Step 3: Publishing GitHub release" -Additional @{ "Details" = "ReleaseId: $($result.ReleaseId)" }
                ...
                if ($PSCmdlet.ShouldProcess($result.ReleaseId, "Publish GitHub release")) {
                    ...
                    if ($publishResult.Success) {
                        ...
                        Write-SafeInfoLog -Message "Release published successfully" -Additional @{ "Details" = "ReleaseId: $($result.ReleaseId)" }
                    } else {
                    ...
                    Write-SafeInfoLog -Message "Release publication simulation successful" -Additional @{ "Details" = "ReleaseId: $($result.ReleaseId) (WhatIf)" }
                }
            }
            ...
            Write-SafeInfoLog -Message "Smart release completed" -Additional @{ "Details" = "Success: $($result.Success), Duration: $($result.Duration.TotalSeconds)s" }
            ...
            if ($result.RollbackInfo.ReleaseToDelete) {
                Write-SafeWarningLog -Message "Rolling back: Deleting draft release" -Additional @{ "Details" = "ReleaseId: $($result.RollbackInfo.ReleaseToDelete)" }
                try {
            ...
            Write-SafeErrorLog -Message "Smart release failed" -Additional @{ "Details" = "Error: $($_.Exception.Message)" }
            throw
        }
        finally {
            Pop-Location
...
function New-GitHubDraftRelease {
    ...
    begin {
        Write-SafeInfoLog -Message "Creating GitHub draft release" -Additional @{ "Details" = "Version: $Version" }
    ...
    process {
        try {
            ...
            Write-SafeDebugLog -Message "Executing GitHub CLI" -Additional @{ "Details" = "Command: gh $($ghArgs -join ' ')" }
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                $result.Success = $true
                Write-SafeInfoLog -Message "Draft release created successfully" -Additional @{ "Details" = "ReleaseId: $($result.ReleaseId), URL: $($result.HtmlUrl)" }
            } else {
            ...
            Write-SafeErrorLog -Message "Failed to create draft release" -Additional @{ "Details" = "Error: $($_.Exception.Message)" }
            throw
        }
        finally {
            Pop-Location
...
function Publish-GitHubRelease {
    ...
    begin {
        Write-SafeInfoLog -Message "Publishing GitHub release" -Additional @{ "Details" = "ReleaseId: $ReleaseId" }
    ...
    process {
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-SafeInfoLog -Message "Release published successfully" -Additional @{ "Details" = "ReleaseId: $ReleaseId, Latest: $($MarkAsLatest.IsPresent)" }
            } else {
            ...
            $result.Success = $false
            $result.ErrorMessage = $_.Exception.Message
            Write-SafeErrorLog -Message "Failed to publish release" -Additional @{ "Details" = "Error: $($_.Exception.Message)" }
            throw
        }
        finally {
            Pop-Location
...
function Remove-GitHubRelease {
    ...
    try {
        ...
        if ($releaseInfo) {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-SafeInfoLog -Message "Release deleted successfully" -Additional @{ "Details" = "ReleaseId: $ReleaseId" }
                return $true
            }
        }
        
        Write-SafeWarningLog -Message "Failed to delete release" -Additional @{ "Details" = "ReleaseId: $ReleaseId" }
        return $false
    }
    catch {
        Write-SafeErrorLog -Message "Error deleting release" -Additional @{ "Details" = "ReleaseId: $ReleaseId, Error: $($_.Exception.Message)" }
        return $false
    }
    finally {
        Pop-Location
